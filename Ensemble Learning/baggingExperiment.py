...
def get_bagged_tree(m, T_bag, train, key, globaldf):
...
def logger(h_bag, h_tree, inum):
    if inum == 0:
        ...
        h_bags.to_csv('h_bags'+str(numcsv)+'.csv', index=False)
        h_trees.to_csv('h_trees'+str(numcsv)+'.csv', index=False)
    else:
        numcsv = int(len(glob.glob('*.csv'))/2 - 1)
        bagname = 'h_bags'+str(numcsv)+'.csv'
        treename = 'h_trees'+str(numcsv)+'.csv'
...
def progress_report(i_time, i, itera):
    print('\nProgress----------------------------------------')
    print(f'Number of bagged trees complete: {i+1}/{itera}')
